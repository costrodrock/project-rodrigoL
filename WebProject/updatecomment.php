...
<?php
    ...
    switch (true) {
            case isset($_POST['likeBtn']):
                ...
                $sql = "SELECT likes, liked_by, disliked_by FROM comments WHERE commentID = ?";
                ...
                if (!empty($liked_by) && in_array($_SESSION['userID'], $liked_by)) {
                    ...
                    // If the user has not already liked the comment, update the 'liked_by' column of the row in the 'comments' table
                ...
                // Update the 'likes', 'liked_by', and 'disliked_by' columns in the 'comments' table
                $sql = "UPDATE comments SET likes = ?, liked_by = ?, disliked_by = ? WHERE commentID = ?";
                $stmt = mysqli_prepare($connection, $sql);
                mysqli_stmt_bind_param($stmt, 'isss', $likes, $serialized_liked_by, $serialized_disliked_by, $commentID);
                mysqli_stmt_execute($stmt);
            break;
            case isset($_POST['dislikeBtn']):
                ...
                if (!empty($disliked_by) && in_array($_SESSION['userID'], $disliked_by)) {
                    ...
                    // If the user has not already disliked the comment, update the 'disliked_by' column of the row in the 'comments' table
                    $likes--; // Decrement likes count
